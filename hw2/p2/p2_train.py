...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    plt.xlabel('Epoch')
    plt.savefig(logfile_dir+'\\'+'train_loss.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.savefig(logfile_dir+'\\'+'val_acc.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.savefig(logfile_dir+'\\'+'val_loss.png')
    plt.clf()
    pass
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            ...
            for batch,data in enumerate(val_loader):
                ...
                sys.stdout.flush()
                imgs, labs = data['images'].to(device), data['labels'].to(device)
                predict = model(imgs)
                loss = criterion(predict, labs)
                val_correct += torch.sum(torch.argmax(predict, dim=1) == labs)
                val_loss += loss.item()
            ######################### TODO End ##########################
