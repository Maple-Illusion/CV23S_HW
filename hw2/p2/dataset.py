...
class CIFAR10Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(self.dataset_dir+'\\'+str(self.image_names[index]))
        ...
        # if self.split == 'train':
        #     return {
        #         'images': torch.from_numpy(img).type(torch.LongTensor), 
        #         'labels': torch.from_numpy(label).type(torch.LongTensor)
        #     }
        # elif self.split == 'test' or self.split == 'val':
        #     return{
        #         'images': torch.from_numpy(img).type(torch.Tensor)
        #     }
