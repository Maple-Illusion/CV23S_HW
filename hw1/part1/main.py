import numpy as np
import cv2
import argparse
from DoG import Difference_of_Gaussian


def plot_keypoints(img_gray, keypoints, save_path):
    img = np.repeat(np.expand_dims(img_gray, axis = 2), 3, axis = 2)
    for y, x in keypoints:
        cv2.circle(img, (y, x), 5, (0, 0, 255), -1)
    cv2.imwrite(save_path, img)

def main():
    parser = argparse.ArgumentParser(description='main function of Difference of Gaussian')
    parser.add_argument('--threshold', default=3.0, type=float, help='threshold value for feature selection')
    parser.add_argument('--image_path', default='./testdata/2.png', help='path to input image')
    args = parser.parse_args()
    args.image_path = r'C:\Users\ataraxia\Documents\AV_LAB\NTU_CV\hw1_material\part1\testdata\2.png'
    print('Processing %s ...'%args.image_path)
    img = cv2.imread(args.image_path, 0).astype(np.float32)
    # print(img.dtype)
    # np.savetxt('P1.txt',img)

    # cv2.namedWindow('test',cv2.WINDOW_AUTOSIZE)
    # cv2.imshow('test',np.uint8(img))

    DoG = Difference_of_Gaussian(args.threshold)
    keypoints = DoG.get_keypoints(img)
    plot_keypoints(img,keypoints,'./testdata/2_3.png')
    # plot_keypoints(img,Difference_of_Gaussian(img,args.threshold),r'C:\Users\ataraxia\Documents\AV_LAB\NTU_CV\hw1_material\part1\testdata\1_test.png')
    # print(img)
    
    # cv2.waitKey()
    ### TODO ###


if __name__ == '__main__':
    
    main()