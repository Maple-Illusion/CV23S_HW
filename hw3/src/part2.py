...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            (corners,ids,rejectedImgPoints) = aruco.detectMarkers(frame,dictionary = arucoDict,parameters = arucoParameters)
            # # TODO: 2.find homograpy
            # # function call to solve_homography()
            # # print(corners[0])
            H = solve_homography(ref_corns, corners[0][0])
            # # TODO: 3.apply backward warp
            # # function call to warping()
            xmin=np.min(corners[0][0][:,0]).astype(np.int16)
            xmax=np.max(corners[0][0][:,0]).astype(np.int16)
            ymin=np.min(corners[0][0][:,1]).astype(np.int16)
            ymax=np.max(corners[0][0][:,1]).astype(np.int16)
